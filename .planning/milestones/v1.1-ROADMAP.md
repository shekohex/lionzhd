...
Milestone v1.1 made discovery feel user-owned without mutating shared taxonomy. It shipped per-user category controls, ignored discovery filtering, searchable navigation, canonical `/search` behavior, detail-page category context, refreshed browser proof, and the final Phase 16 history-replay gap closure needed for re-audit pass.
...
**Key Decisions:**
- Keep category personalization as a user-scoped overlay on shared taxonomy instead of mutating global category data.
- Keep browse, ignored-state, and search behavior on shared read paths so discovery stays aligned across web and mobile.
- Keep `/search` URL-authoritative for media type, sort, and pagination so refresh and history replay stay deterministic.
- Reuse one live browser auth bootstrap across search, navigation, ignored recovery, and detail-page proof.

**Issues Resolved:**
- Delivered per-user category ordering, pinning, hiding, and reset across movies and series.
- Restored ignored discovery recovery flows and searchable category navigation proof against the live UI.
- Added detail-page category chips with browse handoff for both media types.
- Closed the `/search` history restoration regression so milestone re-audit passed.

**Issues Deferred:**
- Auto-download control enhancements remain for a future milestone.
- External-user quota and audit controls remain for a future milestone.
- Bulk category management and onboarding guidance remain for a future milestone.

**Technical Debt Incurred:**
- Existing `console.log` placeholders remain in `resources/js/pages/movies/show.tsx` and `resources/js/pages/series/show.tsx`.
