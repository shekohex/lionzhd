...
final class UpdateSeriesMonitoringRequest extends ApiRequest
{
    /**
    ...
    public function withValidator(Validator $validator): void
    {
        $validator->after(function (Validator $validator): void {
            ...
            if ($scheduleType === MonitorScheduleType::Daily->value) {
                $dailyTime = $this->exists('schedule_daily_time') ? $this->input('schedule_daily_time') : $monitor?->schedule_daily_time;
            ...
            if ($scheduleType === MonitorScheduleType::Weekly->value) {
                $weeklyDays = $this->exists('schedule_weekly_days') ? $this->input('schedule_weekly_days') : $monitor?->schedule_weekly_days;

                if (! is_array($weeklyDays) || $weeklyDays === []) {
                    $validator->errors()->add('schedule_weekly_days', 'Weekly schedule requires at least one day.');
                }

                $weeklyTime = $this->exists('schedule_weekly_time') ? $this->input('schedule_weekly_time') : $monitor?->schedule_weekly_time;
    ...
    private function resolvedScheduleType(?SeriesMonitor $monitor): ?string
    {
        ...
        return $monitor?->schedule_type instanceof MonitorScheduleType ? $monitor->schedule_type->value : null;
    }
